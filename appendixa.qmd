...
If $X \sim \N(\mu, \Sigma),$ then for any $n$-tuple of real numbers $(a_i, \dots, a_k),$ we have that $a_1X_1 + \dots + a_kX_k$ is Normal, where we take constants to be Normal with zero variance.

If $X \sim \N(\mu, \Sigma)$ and $\cov(X_i, X_j) = 0,$ then $X_i$ and $X_j$ are independent. That is, uncorrelatedness and independence imply each other within an MVN.